...
# Critical: When creating a new monthly sheet, we must add headers so the code knows where to write.
SHEET_HEADERS = [
    ...
    "Referral Credit", "Net Amount", "Extra", "Earnings"
]
...
# --- DRIVE FOLDER LOGIC (UPDATED FOR REQ 7) ---
...
def manage_drive_folders(service, doc_type, date_obj):
    ...
    if mmm_yy.startswith("Jan") and mmm_yy[3] != '-': # Fix for Windows/Linux naming consistency if needed
         mmm_yy = date_obj.strftime("%b-%y")

    # 2. Category Folder: [Nurse/Patient] Agreement YY
    # e.g. "Nurse Agreement 26" or "Patient Agreement 26"
...
# --- CONNECT TO GOOGLE SHEETS (CRITICAL UPDATE FOR DYNAMIC WORKBOOKS) ---
def get_active_sheet_client(drive_service, date_obj):
    """
    1. Determines Year (26) and Month (Jan-26).
    2. Checks 'Vesak Invoice History' folder for 'Vesak_Invoice_26'.
    3. Creates Workbook if missing.
    4. Checks Workbook for 'Jan-26' tab.
    5. Creates Tab if missing.
    """
    try:
        creds = get_credentials()
        ...
        yy = date_obj.strftime("%y") # 26
        mmm_yy = date_obj.strftime("%b-%y") # Jan-26
        # Ensure consistency in month name (Title case)
        mmm_yy = mmm_yy.capitalize() 

        wb_name = f"Vesak_Invoice_{yy}"
        
        # 2. Check for Workbook in History Folder
        # We use drive service to find the file ID in the specific folder
        ...
        if files:
            ...
            # Create NEW Workbook
            # client.create creates in root, we must move it
            spreadsheet = client.create(wb_name)
            move_file_to_folder(drive_service, spreadsheet.id, HISTORY_FOLDER_ID)
            # Add initial header to Sheet1 just in case, though we will likely create a named sheet
            try:
...
# Note: get_history_data now needs to know which sheet to look at. 
# We will pass the sheet object dynamically based on the date selected.
# To prevent caching errors with dynamic objects, we remove @st.cache_data for this specific lookup or hash it carefully.
# For simplicity in this complex update, we will fetch fresh data.
...
def save_invoice_to_gsheet(data_dict, sheet_obj):
    ...
    try:
        ...
        # get_history_data.clear() # Removed as we are not using cache for dynamic sheet
        return True
    except Exception as e:
    ...
    if df is not None:
        try:
            ...
            with tab3:
                ...
                reprint_date = st.date_input("Select Invoice Date for Reprint:", value=datetime.date.today())
                reprint_sheet_obj = get_active_sheet_client(drive_service, reprint_date)
