...
def render_invoice_ui(df_main, mode="standard"):
    ...
    with col2:
        ...
        # --- CRITICAL FIX 1: NOTES POPULATION (NAN CHECK) ---
        val_notes = row.get('Notes', '')
        # Check if value is NaN or string 'nan'
        if pd.isna(val_notes) or str(val_notes).strip().lower() == 'nan':
            notes_default = ""
        else:
            notes_default = str(val_notes)
...
if raw_file_obj:
    try:
        ...
        with tab5:
            # --- CRITICAL FIX 2: NURSE MANAGEMENT UI UPDATE ---
